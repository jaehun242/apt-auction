import type { AuctionHistoryEvent, AuctionItem } from '../types/auction'
import { getSeoulDateKey, getSeoulWeekBounds } from '../utils/auction'
...
const dateFromToday = (days: number) => addDays(getSeoulDateKey(), days)
const dateFromWeekStart = (days: number) => addDays(getSeoulWeekBounds().start, days)

const lifecycle = (
  firstSeenScope: 'this-week' | 'past',
  failedScope: 'this-week' | 'past' | null,
  failedCount: number,
) => {
  const firstSeenAt = firstSeenScope === 'this-week' ? dateFromWeekStart(0) : dateFromToday(-14)
  const failedAt = failedScope === 'this-week' ? dateFromWeekStart(0) : failedScope === 'past' ? dateFromToday(-10) : null
  const history: AuctionHistoryEvent[] = [
    { date: firstSeenAt, type: 'FIRST_SEEN', previousValue: null, newValue: '처음 수집' },
  ]
  if (failedAt) history.push({ date: failedAt, type: 'FAILED', previousValue: Math.max(0, failedCount - 1), newValue: failedCount })
  return {
    firstSeenAt,
    lastSeenAt: dateFromToday(0),
    failedAt,
    statusUpdatedAt: failedAt ?? firstSeenAt,
    history,
  }
}
...
export const sampleAuctions: AuctionItem[] = [
  {
    ...
    appraisalPrice: 2_800_000_000, minimumPrice: 2_240_000_000, failedCount: 1, auctionDate: dateFromToday(0), ...lifecycle('this-week', 'this-week', 1), status: 'urgent',
  ...
  },
  {
    ...
    appraisalPrice: 1_850_000_000, minimumPrice: 1_184_000_000, failedCount: 2, auctionDate: dateFromToday(12), ...lifecycle('past', 'this-week', 2), status: 'failed',
  ...
  },
  {
    ...
    appraisalPrice: 1_620_000_000, minimumPrice: 1_620_000_000, failedCount: 0, auctionDate: dateFromToday(18), ...lifecycle('this-week', null, 0), status: 'new',
  ...
  },
  {
    ...
    appraisalPrice: 620_000_000, minimumPrice: 396_800_000, failedCount: 2, auctionDate: dateFromToday(1), ...lifecycle('past', 'past', 2), status: 'urgent',
  ...
  },
  {
    ...
    appraisalPrice: 1_080_000_000, minimumPrice: 864_000_000, failedCount: 1, auctionDate: dateFromToday(-2), ...lifecycle('past', 'past', 1), status: 'closed',
  ...
  },
  {
    ...
    appraisalPrice: 1_350_000_000, minimumPrice: 864_000_000, failedCount: 2, auctionDate: dateFromToday(7), ...lifecycle('past', 'this-week', 2), status: 'urgent',
  ...
  },
  {
    ...
    appraisalPrice: 820_000_000, minimumPrice: 820_000_000, failedCount: 0, auctionDate: dateFromToday(15), ...lifecycle('this-week', null, 0), status: 'new',
  ...
  },
  {
    ...
    appraisalPrice: 710_000_000, minimumPrice: 363_520_000, failedCount: 3, auctionDate: dateFromToday(22), ...lifecycle('past', 'past', 3), status: 'failed',
  ...
  },
  {
    ...
    appraisalPrice: 1_120_000_000, minimumPrice: 896_000_000, failedCount: 1, auctionDate: dateFromToday(8), ...lifecycle('this-week', 'this-week', 1), status: 'failed',
