...
import { formatDate, getAppraisalDiscount, getEffectiveAuctionStatus, getSeoulDateKey, getSummaryMetrics } from './utils/auction'
...
export default function AuctionApp() {
  ...
  const dataDate = formatDate(getSeoulDateKey())
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        ...
        <div className="header-meta"><span>서울·부산</span><span className="meta-divider" /><span>데이터 기준일 <strong>{dataDate}</strong></span><span className="sample-badge">SAMPLE DATA</span></div>
      </header>
      <main>
        <section className="notice" aria-label="샘플 데이터 안내"><strong>현재 화면은 기능 확인용 샘플 데이터입니다.</strong><span>실제 법원 경매 물건·사건번호·권리관계 정보가 아니며<br className="mobile-break" /> 투자 판단에 사용할 수 없습니다.</span></section>
        ...
        <section className="summary-grid" aria-label="경매 현황 요약">
          {summaryCards.map(({ label, value, icon: Icon, help }) => (
            <article className="summary-card" key={label}>
              ...
              <div><span className="summary-label">{label}<InfoTip label={label} wide>{help}</InfoTip></span><strong>{value}<small>건</small></strong></div>
            </article>
          ))}
        </section>
