...
const assumedAmount = (value: number | null) => value === null ? '확인 필요' : formatKoreanCurrency(value)
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="detail-modal" role="dialog" aria-modal="true" aria-labelledby="detail-title">
        ...
        <div className="detail-content">
          ...
          <DetailSection title="법원 원문 정보" kind="fact"><div className="document-grid">{documents.map(({ label, key }) => item.documents[key] ? <a key={key} href={item.documents[key]!} target="_blank" rel="noreferrer"><FileText size={17} /><span>{label}</span><ExternalLink size={13} /></a> : <button key={key} type="button" disabled><FileText size={17} /><span>{label}<small>안정적 링크 확인 전 미제공</small></span></button>)}</div></DetailSection>

          <DetailSection title="권리분석" kind="analysis"><DataGrid rows={[
            ['말소기준권리', valueOrCheck(item.rightsAnalysis.benchmarkRight)], ['말소기준권리 일자', formatDate(item.rightsAnalysis.benchmarkRightDate)],
            ['선순위 임차인', valueOrCheck(item.rightsAnalysis.seniorTenant)], ['후순위 임차인', valueOrCheck(item.rightsAnalysis.juniorTenant)],
            ['매각 후 존속권리', valueOrCheck(item.rightsAnalysis.survivingRights)], ['예상 인수금액', assumedAmount(item.rightsAnalysis.assumedAmount)],
            ['권리위험도', riskLabel(item.rightsAnalysis.riskLevel)], ['분석 메모', valueOrCheck(item.rightsAnalysis.notes)],
          ]} /><p className="analysis-disclaimer"><ShieldCheck size={15} /> 자동 권리분석을 제공하지 않습니다. 실제 입찰 전 법원 원문 및 등기사항을 확인하세요.</p></DetailSection>

          <DetailSection title="점유 및 명도" kind="analysis"><DataGrid rows={[
            ['현재 점유자', valueOrCheck(item.occupancyAnalysis.occupant)], ['대항력 있는 임차인 여부', valueOrCheck(item.occupancyAnalysis.tenantOpposability)],
            ['명도위험도', riskLabel(item.occupancyAnalysis.evictionRisk)], ['확인사항', valueOrCheck(item.occupancyAnalysis.notes)],
          ]} /></DetailSection>

          <DetailSection title="추가비용" kind="analysis"><DataGrid rows={[
            ...
            ['예상 인수금액', assumedAmount(item.rightsAnalysis.assumedAmount)], ['기타 추가확인사항', valueOrCheck(item.additionalCosts.notes)],
          ]} /></DetailSection>
        </div>
      </section>
    </div>
...
function DetailSection({ title, kind, children }: { title: string; kind: 'fact' | 'analysis'; children: React.ReactNode }) {
  return <section className="detail-section"><div className="section-title"><h3>{title}</h3><span className={`section-kind ${kind}`}>{kind === 'fact' ? '법원 공개 / 사실 데이터' : '확인 필요'}</span></div>{children}</section>
}
