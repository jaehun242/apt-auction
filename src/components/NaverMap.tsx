...
function infoWindowContent(item: AuctionItem) {
  ...
  return `<div class="naver-info">
    <div class="naver-info__top"><span>${escapeHtml(statusLabel(effectiveStatus))}</span><small>샘플 데이터</small></div>
...
export function NaverMap({ items, city, selectedId, onSelect, onOpenDetail }: NaverMapProps) {
    ...
    loadNaverMap(clientId).then(() => {
      ...
      if (!mapRef.current) {
        mapRef.current = new maps.Map(containerRef.current, {
          center: new maps.LatLng(36.36, 127.8), zoom: 7, minZoom: 6, zoomControl: true,
          zoomControlOptions: { position: maps.Position.TOP_RIGHT }, scaleControl: false,
  ...
  useEffect(() => {
    ...
    items.forEach((item) => {
      ...
      const marker = new maps.Marker({
        position, map, title: item.apartmentName,
        icon: {
          content: `<div class="auction-marker ${selected ? 'is-selected' : ''}" style="--marker-color:${markerColors[effectiveStatus]}"><span>${effectiveStatus === 'closed' ? '종료' : item.failedCount ? `${item.failedCount}회` : '신건'}</span></div>`,
    ...
    else if (items.length === 1) map.morph(new maps.LatLng(items[0].latitude, items[0].longitude), 14)
  ...
  return (
    <div className="map-frame">
      ...
      <div className="map-legend"><span><i style={{ background: markerColors.new }} />신건</span><span><i style={{ background: markerColors.failed }} />유찰</span><span><i style={{ background: markerColors.urgent }} />입찰임박</span><span><i style={{ background: markerColors.closed }} />종료</span></div>
    </div>
  )
}
