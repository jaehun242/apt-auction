...
import { COURT, DROP_GUARD_RATIO, PATHS } from './config.mjs'
...
try {
  const previousState = await readJson(PATHS.state, { schemaVersion: 1, lastSuccessfulCollectedAt: null, items: {} })
  ...
  const merged = mergeWithState(geocoded.items, previousState, collectedAt)
  ...
  const metadata = {
    ...
    geocoding: {
      runnerOnly: true,
      enabled: geocoded.enabled,
      located: items.filter((item) => item.latitude != null && item.longitude != null).length,
